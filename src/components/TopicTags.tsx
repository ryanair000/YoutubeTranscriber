  ...
  return (
    <section className="mt-16">
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        {topics.map((topic, i) => {
          const colors = colorMap[topic.colorClass] || colorMap.purple;
          return (
            <span
              key={i}
              className={`inline-flex items-center rounded-full border px-4 py-2 text-sm font-medium transition-transform hover:scale-105 ${colors}`}
            >
              {topic.label}
            </span>
          );
